...
class Barclays extends lC_Addon {
  /*
  * Class constructor
  */
  public function Barclays() {    
    ...
    */     
    $this->_title = $lC_Language->get('addon_payment_barclays_title');
   /**
    * The addon description used in the addons store listing
    */     
    $this->_description = $lC_Language->get('addon_payment_barclays_description');
   /**
    * The developers name
    */    
    $this->_author = 'Jamie Lea Landers';
   /**
    ...
    */     
    $this->_thumbnail = lc_image(DIR_WS_CATALOG . 'addons/' . $this->_code . '/images/barclays.png');
   /**
  ...
  */
  public function install() {
    ...
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Client ID', 'ADDONS_PAYMENT_" . strtoupper($this->_code) . "_LOGIN', '', 'The Account ID used for the Global Iris payment service.', '6', '0', now())");
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Secret key', 'ADDONS_PAYMENT_" . strtoupper($this->_code) . "_API_TOKEN', '', 'The API Token used for the Global Iris payment service.', '6', '0', now())");
    ...
    //$lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Show Incomplete Orders', 'ADDONS_PAYMENT_" . strtoupper($this->_code) . "_SHOW_INCOMPLETE', '-1', 'Set to \'Yes\' to show incomplete orders on shopping cart page and customer order history page.', '6', '0', 'lc_cfg_use_get_boolean_value', 'lc_cfg_set_boolean_value(array(1, -1))', now())");   
  ...
  public function getKeys() {
    if (!isset($this->_keys)) {
      $this->_keys = array('ADDONS_PAYMENT_' . strtoupper($this->_code) . '_STATUS',
                           'ADDONS_PAYMENT_' . strtoupper($this->_code) . '_LOGIN',
                           'ADDONS_PAYMENT_' . strtoupper($this->_code) . '_API_TOKEN',
